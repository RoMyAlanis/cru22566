...
{
  Connection conexion;
  public SociosDAO () throws ClassNotFoundException, SQLException
  {
  ...
  public Socios mostrarSocio(int _id)
  {
      ...
      Socios s1=null;
       try
       {
           ps=conexion.prepareStatement("select * from socios where idSocios=?");
           ps.setInt(1,_id);
           rs=ps.executeQuery();
      ...
      public boolean ActualizarSocios(Socios s1)
  {
      ...
      try
       {
           ps=conexion.prepareStatement("update socios set nombre=?,apellido=?,direccion=?,localidad=?,fnac=?,email=?,telefono=?,activo=? where idSocios=?");
